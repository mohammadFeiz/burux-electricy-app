export default function Pricing(fetchURL, applicator, interval = 10 * 60 * 1000){
    let $$ = {
        ...
        async CreateDatabase() {
            return new Promise((resolve, reject) => {
                this.ndbrequest = window.indexedDB.open("BRXINTLayerCalcData");
                this.ndbrequest.onerror = (event) => {
                    console.log("error: " + event.target.error);
                    reject(event.target.error);
                };
                
                this.ndbrequest.onsuccess = (event) => {
                    this.db = event.target.result;
                    console.log("success: " + this.db);
                    resolve(this.db);
                };
    
                this.ndbrequest.onupgradeneeded = (event) => {
        ...
        async addDataToTable(tablename, rawdata, dbid) {
            ...
            for (let i = 0; i < length; i++) {
                nrequest = await dbid.transaction(tablename, 'readwrite').objectStore(tablename).put(rawdata[i]);
                if (!isDone) {break;}
                nrequest.onsuccess = function (event) {};
                nrequest.onerror = function (event) {isDone = false;}
            }
            return isDone;
        },
        ...
        async fetchdata() {
            ...
            if (data) {
                let {itemPrices = [],discountRules = [],campaignRules = [],salePeople = '',customer = ''} = data;
                console.log(itemPrices.length + ':' + discountRules.length + ':' + campaignRules.length + ':' + salePeople.length + ':' + customer.length);
                if (itemPrices.length) {
                    this.pricingData.ItemPrices = itemPrices;
                    if (await this.clearAllDatafromTable("itemPrices", this.db).then((value) => { return value; }) === 1) {
                        this.addDataToTable("itemPrices", itemPrices, this.db);
                    }
                    else {
                        return false;
                    }
                ...
                if (discountRules.length) {
                    this.pricingData.DisRules = discountRules;
                    if (await this.clearAllDatafromTable("discountRules", this.db).then((value) => { return value; }) === 1) {
                        this.addDataToTable("discountRules", discountRules, this.db);
                    }
                    else {
                        return false;
                    }
                }
                if (campaignRules.length) {
                    this.pricingData.CampRules = campaignRules;
                    if (await this.clearAllDatafromTable("campaignRules", this.db).then((value) => { return value; }) === 1) {
                        this.addDataToTable("campaignRules", campaignRules, this.db);
                    }
                    else {
                        return false;
                    }
                ...
                if (customer) {
                    this.pricingData.Customer = customer;
                    if (await this.clearAllDatafromTable("customer", this.db).then((value) => { return value; }) === 1) {
                        this.addDataToTable("customer", [customer], this.db);
                    }
                    else {
                        return false;
                    }
                ...
                if (salePeople) {
                    this.pricingData.SlpCode = salePeople;
                    if (await this.clearAllDatafromTable("slpCode", this.db).then((value) => { return value; }) === 1) {
                        this.addDataToTable("slpCode", [salePeople], this.db);
                    }
                    else {
                        return false;
                    }
        ...
        clearAllDatafromTable(tablename, dbid) {
            return new Promise(function (resolve, reject) {
                if (dbid && dbid.objectStoreNames && dbid.objectStoreNames.contains(tablename)) {
        ...
        startservice() {
            let result = new Promise(async (resolve, reject) => {
                let newdb = await this.CreateDatabase()
                    .then((value) => {
                        console.log(value);
                        return value;
                    })
                ...
                let resfetch = await this.fetchdata();
                if (!resfetch) {
                    resfetch = await this.getAllFromDataBase();
                }
                this.updateTimer = setTimeout(this.refresh, this.updateInterval, this);
        ...
        async refresh() {
            ...
            let isgotfromurl = await this.fetchdata();
            if (!isgotfromurl) {
                let res = await this.getAllFromDataBase();
            }
            console.log((new Date()) + ": update finished");
            this.updateTimer = setTimeout(this.refresh, 10000, this);
        },
        ...
        CalculatePriceDiscount(docdate, cardcode, cardgroupcode, lines,
            ...
            for (let item of lines) {
                ...
                if (price != null) {
                    res.SecondQty = res.ItemQty / Math.min(price[0] && price[0].qtyinBox !== undefined?1:0, 1);
                    res.UnitOfMeasure = price && price[0]?price[0].saleMeasureUnit:undefined;
                    res.ItemGroupCode = price && price[0]?price[0].groupCode:undefined;
                }
        ...
        CalculatePriceDiscountByCampaign(MD, campaignRules, KeepOthers = false) {
            ...
            for (let item of itemofraws) {
                if (item.CampaignDetails) { item.CampaignDetails.Status = -2; }
            };
            let shortrules = [];
                        ...
                        if (temp.indexOf("," + line.ItemCode + ",") > -1 && !PrssdLine.indexOf(linenum) > -1) {
                            ...
                            if (item.lineMaxReqQty != null) {
                                if (br.qty == item.lineMaxReqQty || br.isFull) {
                                    ...
                                    br.isFull = true;
                                    br.status = 0;
                                    if (!KeepOthers) this.ZerolineMarketing(line);
                                }
                    ...
                    if (item.lineMinReqQty && item.lineMinReqQty != 0)
                        if (br.qty < item.lineMinReqQty) {
                            for (let itemb1 of br.lines) {
                                ...
                                if (!KeepOthers) this.ZerolineMarketing(results.MarketingLines[itemb1]);
                            }
                            br.disQty = 0;
                            br.isExist = false;
                        }
        ...
        GetPriceListInfo(ItemCodes, Items, DisRules, campaignRules, SlpCodes = null, CurrentDoc = null, CardCode = null
            ...
            let PaymentDic = doctocalc && doctocalc.marketingdetails?doctocalc.marketingdetails.DocumentDiscountPercent : 0;
            ...
            //بررسی متد کمپین
            if (doctocalc && doctocalc.marketingdetails?doctocalc.marketingdetails.Campaign:false) {
    ...
    return {
        startservice:$$.startservice.bind($$)
    }
}
